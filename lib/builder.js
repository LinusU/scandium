...
export async function createZipFile (directory, { customEntrypoint, platform, skipNodeModules, sshKey }) {
  const cleanupFiles = []
  const removeUnloadCleanupHandler = unload.add(() => {
    for (const file of cleanupFiles) rmFileSync(path.join(directory, file))
  })
  ...
  if (!hasDockerignore) {
    const ignore = DEFAULT_IGNORE + await generateDockerignore(directory)
  ...
  if (sshKey) {
    ...
    imageId = (await execa('docker', ['build', '--platform', platform, '--quiet', '--build-arg', 'SSH_PRIVATE_KEY', '--build-arg', 'SSH_PUBLIC_KEY', '--file', dockerfilePath, '.'], { cwd: directory, env: { SSH_PRIVATE_KEY: sshKeyPrivate, SSH_PUBLIC_KEY: sshKeyPublic } })).stdout
  } else {
    imageId = (await execa('docker', ['build', '--platform', platform, '--quiet', '--file', dockerfilePath, '.'], { cwd: directory })).stdout
  }
