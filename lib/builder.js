...
const rmFile = path => fs.unlinkSync(path)
...
function cleanup (directory) {
   rmFile(path.join(directory, 'scandium-dockerfile'))
   rmFile(path.join(directory, 'scandium-entrypoint.js'))

   rmFile(path.join(directory, 'scandium-clean-package.json'))
   rmFile(path.join(directory, 'scandium-clean-package-lock.json'))
}

exports.createZipFile = async function (directory) {
  const removeUnloadCleanupHandler = unload.add(() => cleanup(directory))
