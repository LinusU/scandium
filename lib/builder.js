...
const filesToRemove = directory => SCANDIUM_FILES.map(file => path.join(directory, file))

exports.createZipFile = async function (directory) {
  const removeUnloadCleanupHandler = unload.add(() => filesToRemove(directory).forEach(fs.unlinkSync))
  ...
  await Promise.all(filesToRemove(directory).map(rmFile))
