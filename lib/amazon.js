...
function lambdaRuntime (runtime) {
  return (runtime === 'rust') ? 'provided' : 'nodejs12.x'
}
...
exports.createFunction = function ({ code, functionName, handler, role, runtime, environment, onFailedAttempt }) {
  const params = {
    Code: code,
...
function updateRuntime ({ functionName, handler, runtime }) {
  const params = {
    FunctionName: functionName,
    Handler: handler,
    Runtime: lambdaRuntime(runtime)
...
exports.updateFunction = async function ({ code, functionName, handler, runtime }) {
  const params = {
    FunctionName: functionName,
    Publish: true,
    ...code
  ...
  if (result.Runtime !== lambdaRuntime(runtime) || result.Handler !== handler) {
    return updateRuntime({ functionName, handler, runtime })
  }
