...
async function updateRuntime ({ functionName, handler, onFailedAttempt }) {
  const params = {
    FunctionName: functionName,
    Handler: handler,
    Runtime: defaultParams.Runtime
...
export async function updateFunction ({ code, functionName, handler, onFailedAttempt }) {
  const params = {
    FunctionName: functionName,
    Publish: true,
    ...code
  ...
  if (result.Runtime !== defaultParams.Runtime || result.Handler !== handler) {
    return updateRuntime({ functionName, handler, onFailedAttempt })
  }
