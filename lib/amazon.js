...
export async function createFunction ({ code, functionName, handler, role, environment, onFailedAttempt, vpcConfig }) {
  const params = {
    Code: code,
...
async function updateFunctionConfiguration ({ functionName, handler, onFailedAttempt, vpcConfig }) {
  ...
  return (await pRetry(async () => await lambda.send(new UpdateFunctionConfigurationCommand(params)), { minTimeout: 2000, factor: 3, retries: 5, onFailedAttempt })).FunctionArn
}

export async function updateFunction ({ code, functionName, handler, onFailedAttempt, vpcConfig }) {
  const params = {
    FunctionName: functionName,
    Publish: true,
    ...code
  ...
  const result = await pRetry(async () => await lambda.send(new UpdateFunctionCodeCommand(params)), { minTimeout: 2000, factor: 3, retries: 5, onFailedAttempt })

  if (result.Runtime !== defaultParams.Runtime || result.Handler !== handler || vpcConfig != null) {
    return updateFunctionConfiguration({ functionName, handler, onFailedAttempt, vpcConfig })
  }
