...
function verifyZipFile (zipFile, { usingS3 }) {
  ...
  if (usingS3) {
    ...
    if (zipFile.byteLength >= MAX_LAMBDA_ZIP_SIZE) throw new Error(`Lambda size exceeded, ${bytes.format(zipFile.byteLength)}, lamda max zip size is ${bytes.format(MAX_LAMBDA_ZIP_SIZE)}, use --bucket to use s3`)
  }
}
