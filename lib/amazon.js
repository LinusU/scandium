...
function verifyZipFile (zipFile, {sizeLimit}) {
  if (!zipFile) throw new Error('Missing zipFile')
  if (sizeLimit && zipFile.byteLength >= sizeLimit) throw new Error(`Lambda size exceeded, ${bytes.format(zipFile.byteLength)}, current max size is ${bytes.format(MAX_LAMBDA_ZIP_SIZE)}`)
}
...
exports.createFunction = function ({ zipFile, functionName, role, environment, onFailedAttempt, bucket, s3Key }) {
  verifyZipFile(zipFile, { sizeLimit: bucket ? null : MAX_LAMBDA_ZIP_SIZE })
