...
function verifyZipFile (zipFile, { usingS3 }) {
  if (!zipFile) throw new Error('Missing zipFile')
  if (usingS3) {
    if (zipFile.byteLength >= MAX_S3_ZIP_SIZE) throw new Error(`Lambda size exceeded, ${bytes.format(zipFile.byteLength)}, max zip size for s3 deployments is ${bytes.format(MAX_S3_ZIP_SIZE)}`)
  } else {
    if (zipFile.byteLength >= MAX_LAMBDA_ZIP_SIZE) throw new Error(`Lambda size exceeded, ${bytes.format(zipFile.byteLength)}, lambda max zip size is ${bytes.format(MAX_LAMBDA_ZIP_SIZE)}, use --bucket to use S3`)
  }
}
...
function getCode ({ zipFile, bucket, s3Key }) {
  return s3Key ? { S3Key: s3Key, S3Bucket: bucket } : { ZipFile: zipFile }
}

exports.createFunction = function ({ zipFile, functionName, role, environment, onFailedAttempt, bucket, s3Key }) {
  verifyZipFile(zipFile, { usingS3: Boolean(s3Key) })

  const params = {
    Code: getCode({ zipFile, bucket, s3Key }),
...
exports.updateFunction = async function ({ zipFile, functionName, bucket, s3Key }) {
  verifyZipFile(zipFile, { usingS3: s3Key })

  const params = {
    FunctionName: functionName,
    Publish: true,
    ...getCode({ zipFile, bucket, s3Key })
  }
