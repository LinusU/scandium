...
import awsHasRegion from 'aws-has-region'
...
export const createLambdaFunction = {
  ...
  task: async (ctx, task) => {
    ...
    ctx.lambdaArn = await amazon.createFunction({ code: ctx.code, functionName: ctx.name, handler: ctx.entrypointHandler, role: ctx.roleArn, environment: ctx.environment, onFailedAttempt, vpcConfig: ctx.vpcConfig })
...
export const updateLambdaFunction = {
  ...
  task: async (ctx, task) => {
    ...
    try {
      ctx.lambdaArn = await amazon.updateFunction({ code: ctx.code, functionName: ctx.name, handler: ctx.entrypointHandler, onFailedAttempt, vpcConfig: ctx.vpcConfig })
