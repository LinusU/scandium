...
exports.createLambdaFunction = {
  ...
  task: async (ctx, task) => {
    ctx.lambdaArn = await amazon.createFunction({ zipFile: ctx.zipFile, functionName: ctx.args['<name>'], role: ctx.roleArn, env: ctx.env })
...
exports.updateLambdaFunction = {
  ...
  task: async (ctx, task) => {
    try {
      ctx.lambdaArn = await amazon.updateFunction({ zipFile: ctx.zipFile, functionName: ctx.args['<name>'], env: ctx.env })
