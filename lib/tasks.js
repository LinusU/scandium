const fs = require('fs')
const { resolve: resolvePath, isAbsolute: isAbsolutePath } = require('path')
...
exports.parseOptions = {
  ...
  task: async (ctx, task) => {
    ...
    if (ctx['--output']) {
      ctx.outputPath = isAbsolutePath(ctx.args['--output']) ? ctx.args['--output'] : resolvePath(ctx.args['--output'])
    }
  }
}
...
exports.packageApp = {
  ...
  task: async (ctx, task) => {
    ...
    task.title = `App packaged successfully! Final size: ${bytes(ctx.zipFile.byteLength)}`
  }
}
...
exports.saveApp = {
  ...
  task: async (ctx, task) => {
    const { zipFile, outputPath } = ctx
    if (!zipFile) throw new Error('Missing ctx.zipFile')
    await writeFile(outputPath, zipFile)
    task.title = `Packaged app saved to ${outputPath}`
  }
}
