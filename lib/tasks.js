...
exports.requireConfirmation = {
  title: 'Confirm',
  enabled: ctx => Boolean(ctx.args['--require-confirmation']),
  task: (ctx, task) => {
    task.title = `Confirm deploy name: ${ctx.name} `
...
exports.updateApiGateway = {
  ...
  task: async (ctx, task) => {
    ...
    task.title = `Updated existing API Gateway with id: ${ctx.id}, name ${ctx.name}`
  }
}
