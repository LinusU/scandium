...
export const packageApp = {
  ...
  task: async (ctx, task) => {
    ...
    try {
      ctx.code = { ZipFile: await builder.createZipFile(process.cwd(), { customEntrypoint: ctx.customEntrypoint, platform: ctx.platform, skipNodeModules: ctx.skipNodeModules, sshKey: ctx.sshKey }) }
    } finally {
