...
const nodejsBuilder = require('./nodejs/builder')
const rustBuilder = require('./rust/builder')
...
exports.detectRuntime = {
  title: 'Detect app runtime environment',
  task: async (ctx, task) => {
    if (fs.existsSync('Cargo.toml')) {
      ctx.runtime = 'rust'
      task.title = 'Detected app runtime environment: Rust'
    } else {
      ctx.runtime = 'nodejs'
      task.title = 'Detected app runtime environment: Node.js'
    }
  }
}
...
exports.packageApp = {
  ...
  task: async (ctx, task) => {
    ...
    try {
      if (ctx.runtime === 'rust') {
        ctx.code = { ZipFile: await rustBuilder.createZipFile(process.cwd(), { customEntrypoint: ctx.customEntrypoint, sshKey: ctx.sshKey }) }
      } else {
        ctx.code = { ZipFile: await nodejsBuilder.createZipFile(process.cwd(), { customEntrypoint: ctx.customEntrypoint, sshKey: ctx.sshKey }) }
      }
    } finally {
...
exports.createLambdaFunction = {
  ...
  task: async (ctx, task) => {
    ...
    ctx.lambdaArn = await amazon.createFunction({ code: ctx.code, functionName: ctx.name, handler: ctx.entrypointHandler, role: ctx.roleArn, runtime: ctx.runtime, environment: ctx.environment, onFailedAttempt })
...
exports.updateLambdaFunction = {
  ...
  task: async (ctx, task) => {
    try {
      ctx.lambdaArn = await amazon.updateFunction({ code: ctx.code, functionName: ctx.name, handler: ctx.entrypointHandler, runtime: ctx.runtime })
...
exports.invokeHooks = {
  title: 'Invoke hooks',
  enabled: (ctx) => Boolean(ctx.args['--hooks']),
  task: async (ctx, task) => {
    const payload = { scandiumInvokeHook: { file: ctx.args['--hooks'], hook: 'deploy' } }
    const { log } = await amazon.invokeLambda({ functionName: ctx.lambdaArn, payload })
...
exports.loadSwaggerDefinition = {
  ...
  enabled: (ctx) => !ctx.args['--no-api-gateway'] && Boolean(ctx.args['--swagger']),
  task: async (ctx, task) => {
    ctx.definition = await swagger.loadSwaggerFile(ctx.args['--swagger'], ctx.name, ctx.lambdaArn, ctx.runtime)
...
exports.generateSwaggerDefinition = {
  ...
  enabled: (ctx) => !ctx.args['--no-api-gateway'] && !ctx.args['--swagger'],
  task: async (ctx, task) => {
    ctx.definition = await swagger.forwardAllDefinition(ctx.name, ctx.lambdaArn, ctx.runtime)
...
exports.createApiGateway = {
  ...
  enabled: (ctx) => !ctx.args['--no-api-gateway'],
  task: async (ctx, task) => {
...
exports.deployApi = {
  ...
  enabled: (ctx) => !ctx.args['--no-api-gateway'] && Boolean(ctx.apiGatewayStage),
  task: async (ctx, task) => {
