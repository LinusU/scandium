...
exports.packageApp = {
  ...
  task: async (ctx, task) => {
    ...
    task.title = `App packaged successfully in ${formatDuration(Date.now() - startedAt)}! Final size: ${bytes.format(ctx.zipFile.byteLength)}`
  }
}
...
exports.saveApp = {
  ...
  task: async (ctx, task) => {
    if (!ctx.zipFile) throw new Error('Missing ctx.zipFile')
    await writeFile(ctx.outputPath, ctx.zipFile)
...
exports.createLambdaFunction = {
  ...
  task: async (ctx, task) => {
    ...
    ctx.lambdaArn = await amazon.createFunction({ zipFile: ctx.zipFile, functionName: ctx.name, role: ctx.roleArn, environment: ctx.environment, onFailedAttempt })
...
exports.updateLambdaFunction = {
  ...
  task: async (ctx, task) => {
    try {
      ctx.lambdaArn = await amazon.updateFunction({
        zipFile: ctx.zipFile,
...
exports.uploadToS3 = {
  ...
  enabled: (ctx) => Boolean(ctx.args['--bucket']),
  task: async (ctx, task) => {
    const bucketName = ctx.args['--bucket']
    const s3Key = await amazon.uploadToS3({ zipFile: ctx.zipFile, functionName: ctx.name, bucketName })
    ctx.s3Key = s3Key
  }
}
